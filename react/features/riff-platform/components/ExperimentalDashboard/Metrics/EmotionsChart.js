...
class EmotionsChart extends React.Component {
    ...
    render() {
        ...
        if (this.state.emotionsData.length === 0) {
            emptyGraphText = (
                <div className = 'empty-graph-text'>
                    {'It doesn\'t look like the emotions detection module was enabled.'}
                </div>
            );
        }
        ...
        const emotionChartInfo = 'This chart shows how your emotions and the average emotional state '
                                 + 'of all the participants are changed during the meeting.';

        return (
            <ChartCard
                chartCardId = 'EmotionsChartId'
                chartInfo = { emotionChartInfo }
     ...
     *
     * @returns {{ oneWasLoading: boolean, allAreLoaded: boolean, allWereLoaded: boolean }}
     *      Containing three booleans:
